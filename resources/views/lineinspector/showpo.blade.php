                    ...
                    <table class="table table-responsive-md">
                        ...
                        @foreach( $po as $pos)
                            <tr>
                                ...
                                <td> <span class="w-space-no">{{$pos->pn->nom}} </span></td>
                                <td>{{$pos->revision}}	</td>
                                ...
                                <td>
                                    <div class="d-flex">
                                    @if(App\Models\Piece::where('po_id', $pos->id)->count()<2)
                                            <a href="{{url('inspector/po/'.$pos->id.'/piece/add')}}" class="btn btn-primary btn-sm mr-2">Ajouter validation</a>
                                            <a href="{{url('inspector/po/'.$pos->id.'/validation')}}" class="btn btn-secondary btn-sm mr-2">Voir validation</a>
                                    @else
                                        <a href="{{url('inspector/po/'.$pos->id.'/validation')}}" class="btn btn-secondary btn-sm mr-2 float-lg-right">Voir validation</a>
                                    @endif
